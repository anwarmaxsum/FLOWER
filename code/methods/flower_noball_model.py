...
class FLOWERNOBALLModel(BaseModel):
    """Metric-based with random noise to parameters learning model"""
    def __init__(self, opt):
        super(FLOWERNOBALLModel, self).__init__(opt)
    ...
    def incremental_optimize_parameters(self, current_iter):
        ...
        # if max(self.labels).item() in self.prototypes_dict:
        #     l_ball = self.ball_loss_v2(self.zhh_support, self.support_labels, self.ball_params)
        # else:
        #     l_ball = 0
    ...
    def incremental_fine_tune(self, train_dataset, val_dataset, num_epoch, task_id=-1, test_id=-1, tb_logger=None):
        ...
        # print(output.size())
        # print(self.labels)
        self.features_transformer = MLPTransformer().cuda()
        self.transformer_optimizer = torch.optim.SGD(self.features_transformer.parameters(), lr=0.1)
        self.net_g.eval()
