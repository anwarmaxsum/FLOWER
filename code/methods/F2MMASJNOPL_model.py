...
class F2MMASJNOPLModel(BaseModel):
    """Metric-based learning model"""

    def __init__(self, opt):
        super(F2MMASJNOPLModel, self).__init__(opt)
        ...
        self.w = {n: torch.zeros(p.shape).cuda() for n, p in feat_ext.named_parameters() if p.requires_grad}
        
        self.older_params = {n: p.clone().detach().cuda() for n, p in feat_ext.named_parameters() if p.requires_grad}
        
        self.importance = {n: torch.zeros(p.shape).cuda() for n, p in feat_ext.named_parameters() if p.requires_grad}
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.random_noise is not None:
            ...
            for _ in range(self.random_times):
                ...
                #Add PSI Loss
                l_psi =  self.cpr(noise_output)
                l_total += l_psi
                log_dict = {'PSILoss': l_psi.item()}
                self.log_dict.add_dict(log_dict)
            ...
            #Add PSI Loss
            l_psi =  self.cpr(original_output)
            l_total += l_psi
            log_dict = {'PSILoss': l_psi.item()}
            self.log_dict.add_dict(log_dict)
    ...
    def save_importance(self):
        ...
        # print("Importance and older param: ")
        # print(self.importance)
        # print(self.older_params)
        importance_path =  osp.join(self.opt['path']['models'], "importance.pt")
        torch.save(self.importance, importance_path)
        older_params_path =  osp.join(self.opt['path']['models'], "older_params.pt")
