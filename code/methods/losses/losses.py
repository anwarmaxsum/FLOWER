...
class PTFixPNLoss(nn.Module):
    ...
    def forward(self, prototypes, pt_labels, embeddings, labels):
        ...
        print("CEK PT land embeddings labels")
        print(pt_labels)
        print(labels)
...
class PTFixPNLossV2(nn.Module):
    def __init__(self, w, use_cosine=False):
        super(PTFixPNLossV2, self).__init__()
        self.w  = w
        self.use_cosine = use_cosine
        # TODO support cosine similarity
        self.loss_func = nn.CrossEntropyLoss()
...
class IncrementalPTFixPNLossV2(nn.Module):
    def __init__(self, w, omega, n_shots, use_cosine = False):
        super(IncrementalPTFixPNLossV2, self).__init__()
        self.w = w
        self.omega = omega
        self.n_shots = n_shots
        self.use_cosine = use_cosine
    ...
    def forward(self, prototypes, pt_labels, output, labels):
        ...
        for i in unique_labels:
            output_i = output[labels==i]
            proto_i = output_i.mean(dim=0)
            proto_i = torch.reshape(proto_i, (1,output_i.size(dim=1)))
            
            if isFirst:
                isFirst = False
                novel_prototypes = proto_i
            else:
                novel_prototypes = torch.cat((novel_prototypes, proto_i), dim=0)

        prototypes_all = torch.cat((novel_prototypes, prototypes), dim=0)

        # print("cek dimmension: ")
        # print(output.size())
        # print(labels.size())
        # print(novel_prototypes.size())
...
class IncrementalPTFixPNLossV3(nn.Module):
    def __init__(self, w, omega, n_shots, use_cosine = False):
        super(IncrementalPTFixPNLossV3, self).__init__()
        self.w = w
        self.omega = omega
        self.n_shots = n_shots
        self.use_cosine = use_cosine
    ...
    def forward(self, prototypes, pt_labels, output, labels):
        ...
        prototypes_all = torch.cat((prototypes, novel_prototypes.cuda()), dim=0)
        labels_all = torch.cat((pt_labels, torch.tensor(unique_labels).cuda()), dim=0)
        ...
        # estimate = labels_all[logits.argmax(dim=1)]
        estimate = logits.argmax(dim=1)
        ...
        loss = self.loss_func(logits, labels)
        #loss = self.loss_func(estimate, labels)
